"""Retrieves statistics of TA algorithm."""
...
def getparser():
    ...
    parser = argparse.ArgumentParser(
        prog='run', description='Client for Bloodhound server that retrieves'
                                'TA algorithm statistics.')
...
def main():
    ...
    if not args.skip_header:
        print('terms', 'postings', 'traversed', 'lookups', sep=',')
    for query in args.infile:
        ...
        print(len(query.split(' ')),
              ...
              resp_json['stats']['traversed'],
              resp_json['stats']['lookups'],
              sep=',')
