...
TEST_F(DaatTestPair, moving_range)
{
    ...
    auto mr = irkit::moving_range(p.begin(), p.end());
...
TEST_F(DaatTestPair, union_next_posting)
{
    ...
    irkit::UnionRange union_range(postings, weights);
    std::vector<irkit::UnionRange<std::vector<posting>>::docterm> expected_heap;

    posting p = union_range.next_posting();
    ASSERT_EQ(p.doc, 0);
    ASSERT_EQ(p.score, 3);
    expected_heap = {{0, 0}, {2, 1}, {1, 2}};
    ASSERT_THAT(union_range.heap_, ::testing::ElementsAreArray(expected_heap));

    p = union_range.next_posting();
    ASSERT_EQ(p.doc, 0);
    ASSERT_EQ(p.score, 3);
...
TEST(daat_or, postings_weights)
{
    ...
    std::vector<double> weights = {3, 3, 3};
    std::vector<posting> top = irkit::daat_or(postings, 2, weights);
